...
const mapCategoryKeys = (category) => {
    if (!category)
        return null;
    return {
...
export class CategoryService {
    ...
    async delete(id, tx) {
        try {
            ...
            if (e instanceof PrismaClientKnownRequestError && e.code === "P2025") {
                throw new Error(`La categoría con ID ${id} no fue encontrada para borrar.`);
            }
            throw e;
        }
    }
    ...
    async list(page, limit, filter, tx) {
        const where = {};
        Object.assign(where, buildBooleanFilter("is_active", filter.is_active), buildSearchFilter(filter.search, ["name", "description"]));
        const result = await paginate(categoryRepository, { page, limit }, {
            where,
