...
import { leagueRepository, leagueSelectFields, } from "./league.repository";
...
export class LeagueService {
    ...
    async update(id, data, tx) {
        ...
        try {
            const updatedLeague = await leagueRepository.update(id, updateData, tx);
    ...
    async delete(id, tx) {
        try {
            ...
            if (e instanceof PrismaClientKnownRequestError && e.code === "P2025") {
                throw new Error(`La liga con ID ${id} no fue encontrada para borrar.`);
            }
            throw e;
        }
    }
    ...
    async list(page, limit, filter, tx) {
        const where = {};
        Object.assign(where, buildBooleanFilter("is_active", filter.is_active), buildSearchFilter(filter.search, ["name"]));
        const result = await paginate(leagueRepository, { page, limit }, {
            where,
