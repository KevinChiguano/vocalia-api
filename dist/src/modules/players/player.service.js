...
export class PlayerService {
    ...
    async update(dni, data, tx) {
        const updateData = {};
        if (data.name)
            updateData.player_name = data.name;
        if (data.lastname)
            updateData.player_lastname = data.lastname;
        if (data.number)
            updateData.player_number = data.number;
        if (data.dni)
    ...
    async delete(dni, tx) {
        try {
            ...
            if (e instanceof PrismaClientKnownRequestError && e.code === "P2025") {
                throw new Error(`El jugador con dni ${dni} no fue encontrado para borrar.`);
            }
            throw e;
        }
    }
    async getByDni(dni, tx) {
