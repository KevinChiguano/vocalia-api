...
import { sanctionRepository, sanctionSelectFields, } from "./sanction.repository";
import { invalidateStatsByMatch } from "@/utils/cache.stats";
const mapSanctionKeys = (sanction) => {
    if (!sanction)
        return null;
...
export class SanctionService {
    ...
    async update(id, data, tx) {
        ...
        if (data.playerId !== undefined)
            updateData.player_id = data.playerId;
    ...
    async delete(id, tx) {
        try {
            ...
            if (e instanceof PrismaClientKnownRequestError && e.code === "P2025") {
                throw new Error(`La sanción con ID ${id} no fue encontrada para borrar.`);
            }
            throw e;
        }
    }
    ...
    async list(page, limit, filter, tx) {
        ...
        const result = await paginate(sanctionRepository, // Pasamos el Repository al paginador
        { page, limit }, {
            where,
