...
describe("MatchService", () => {
    ...
    describe("create()", () => {
        it("crea un partido correctamente", async () => {
            matchRepository.create.mockResolvedValue({
                match_id: 1,
                match_date: new Date(),
                stage: "final",
                location: "Quito",
    ...
    describe("update()", () => {
        it("actualiza un partido", async () => {
            matchRepository.update.mockResolvedValue({
                match_id: 1,
                match_date: new Date(),
                stage: "semifinal",
                location: "Guayaquil",
        ...
        it("lanza error si el partido no existe", async () => {
            matchRepository.update.mockRejectedValue(new PrismaClientKnownRequestError("Not found", {
                code: "P2025",
                clientVersion: "5.x",
            }));
            await expect(matchService.update(99, { stage: "final" })).rejects.toThrow("El partido con ID 99 no existe.");
        });
    });
    describe("delete()", () => {
        ...
        it("lanza error si no existe", async () => {
            matchRepository.delete.mockRejectedValue(new PrismaClientKnownRequestError("Not found", {
                code: "P2025",
                clientVersion: "5.x",
            }));
            await expect(matchService.delete(99)).rejects.toThrow("El partido con ID 99 no fue encontrado para eliminar.");
        });
    });
    describe("getById()", () => {
