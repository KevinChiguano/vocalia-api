...
describe("StatisticsService", () => {
    describe("playersStatsByTournament()", () => {
        it("retorna datos desde cache si existe", async () => {
            redis.get.mockResolvedValue(JSON.stringify({ items: [], pagination: {} }));
        ...
        it("calcula stats y guarda en cache si no existe", async () => {
            ...
            statisticsRepository.playersByTournament.mockResolvedValue([
                {
                    player_id: 1,
                    player_name: "Juan",
            ...
            statisticsRepository.countPlayersByTournament.mockResolvedValue(1);
            statisticsRepository.goalsByPlayer.mockResolvedValue([
                { player_id: 1, _count: { player_id: 3 } },
            ]);
            statisticsRepository.matchesPlayedByPlayer.mockResolvedValue([
                { player_id: 1, _count: { match_id: 2 } },
            ]);
            statisticsRepository.sanctionsByPlayer.mockResolvedValue([
    ...
    describe("teamsStatsByTournament()", () => {
        it("calcula correctamente tarjetas por equipo", async () => {
            statisticsRepository.teamsByTournament.mockResolvedValue([
                {
                    team_id: 10,
            ...
            statisticsRepository.sanctionsBase.mockResolvedValue([
    ...
    describe("topScorersByTournament()", () => {
        it("retorna goleadores con equipo", async () => {
            statisticsRepository.topScorers.mockResolvedValue([
