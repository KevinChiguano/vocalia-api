...
describe("TournamentTeamService", () => {
    ...
    describe("create()", () => {
        it("crea equipo en torneo", async () => {
            tournamentTeamRepository.create.mockResolvedValue({
        ...
        it("lanza error si ya existe", async () => {
            tournamentTeamRepository.create.mockRejectedValue(new PrismaClientKnownRequestError("Duplicate", {
                code: "P2002",
                clientVersion: "5",
            }));
            await expect(tournamentTeamService.create({ tournamentId: 1, teamId: 2 })).rejects.toThrow("El equipo ya está inscrito en este torneo.");
        });
    });
    describe("update()", () => {
        it("actualiza datos", async () => {
            tournamentTeamRepository.update.mockResolvedValue({
                tournament_team_id: 1,
                tournament_id: 1,
                team: null,
                played: 1,
        ...
        it("lanza error si no hay datos", async () => {
            await expect(tournamentTeamService.update(1, {})).rejects.toThrow("No hay datos válidos para actualizar.");
        });
        it("lanza error si no existe", async () => {
            tournamentTeamRepository.update.mockRejectedValue(new PrismaClientKnownRequestError("Not found", {
                code: "P2025",
                clientVersion: "5",
            }));
            await expect(tournamentTeamService.update(99, { points: 1 })).rejects.toThrow("El registro no existe.");
        });
    });
    describe("delete()", () => {
        it("elimina correctamente", async () => {
            tournamentTeamRepository.delete.mockResolvedValue({});
    ...
    describe("listByTournament()", () => {
        it("retorna equipos del torneo", async () => {
            tournamentTeamRepository.findByTournament.mockResolvedValue([
                {
                    tournament_team_id: 1,
                    tournament_id: 1,
