...
export class ActaService {
  async getByMatchId(matchId: number) {
    ...
    const localPlayers = matchPlayers.filter(
      (p) => p.team.team_id === match.local_team_id
    );
    const awayPlayers = matchPlayers.filter(
      (p) => p.team.team_id === match.away_team_id
    );
     ...
     */
    const vocaliaData = (vocalia.vocalia_data ?? {}) as any;
    ...
    return {
      ...
      captains: {
        local: vocalia.localCaptain
          ? `${vocalia.localCaptain.player_name} ${
              vocalia.localCaptain.player_lastname ?? ""
            }`
          : null,
        away: vocalia.awayCaptain
          ? `${vocalia.awayCaptain.player_name} ${
              vocalia.awayCaptain.player_lastname ?? ""
            }`
          : null,
      },
      ...
      events: {
        ...
        substitutions: substitutions.map((s) => ({
          ...
          playerOut: {
            id: s.player_out,
            name: `${s.playerOut.player_name} ${
              s.playerOut.player_lastname ?? ""
            }`.trim(),
            number: s.playerOut.player_number,
          },
          playerIn: {
            id: s.player_in,
            name: `${s.playerIn.player_name} ${
              s.playerIn.player_lastname ?? ""
            }`.trim(),
