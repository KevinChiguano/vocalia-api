...
describe("GoalService", () => {
  ...
  describe("createBulk()", () => {
    it("lanza error si el arreglo está vacío", async () => {
      await expect(goalService.createBulk([])).rejects.toThrow(
        "El arreglo de goles está vacío."
      );
    });

    it("crea goles en bulk e invalida stats", async () => {
      (goalRepository.createMany as any).mockResolvedValue({ count: 2 });
  ...
  describe("update()", () => {
    it("lanza error si no hay datos para actualizar", async () => {
      await expect(goalService.update(1, {} as any)).rejects.toThrow(
        "No hay datos para actualizar."
      );
    });

    it("actualiza el gol correctamente", async () => {
      (goalRepository.update as any).mockResolvedValue({
        goal_id: 1,
        is_own_goal: true,
        event_time: new Date(),
        player: null,
  ...
  describe("getById()", () => {
    it("devuelve null si no existe", async () => {
      (goalRepository.findById as any).mockResolvedValue(null);
  ...
  describe("delete()", () => {
    it("elimina un gol", async () => {
      (goalRepository.delete as any).mockResolvedValue({});
  ...
  describe("deleteByMatch()", () => {
    it("elimina goles por partido", async () => {
      (goalRepository.deleteByMatchId as any).mockResolvedValue({ count: 3 });
