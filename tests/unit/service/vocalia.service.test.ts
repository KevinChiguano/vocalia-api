...
describe("VocaliaService", () => {
  describe("create()", () => {
    ...
    it("crea vocalía correctamente", async () => {
      ...
      (vocaliaRepository.create as any).mockResolvedValue({
        vocalia_id: 1,
        match_id: 1n,
        vocal_user_id: 2n,
        created_at: new Date(),
  ...
  describe("update()", () => {
    ...
    it("actualiza la vocalía correctamente", async () => {
      (vocaliaRepository.findByMatchAndVocal as any).mockResolvedValue({});
      (vocaliaRepository.update as any).mockResolvedValue({
        vocalia_id: 1,
        match_id: 1n,
        observations: "ok",
        created_at: new Date(),
